...
async def msg(message, x, p, self):
    ...
    if message.startswith(p + "hackban"):
        ...
        channel = get(x.guild.channels, name="case_logs",
                      type=discord.ChannelType.text)
        embed = discord.Embed(title="Ban!", description="<@" + str(msg.author.id) +
                              "> has hackbanned " + str(bannedusers) + " users!\n" + message, color=0x00ff00)
        embed.set_thumbnail(
            url="https://image.freepik.com/free-photo/judge-gavel-hammer-justice-law-concept_43403-625.jpg")
    ...
    if message.startswith(p + "kick"):
        ...
        for member in x.mentions:
            print(member)
            embed = discord.Embed(title="Kick!", description="<@" + str(msg.author.id) +
                                  "> has kicked <@" + str(member.id) + ">! \n\n**" + message + "**", color=0x00ff00)
            if member.id == msg.author.id:
                return
    ...
    if message.startswith(p + "warn"):
        ...
        for member in x.mentions:
            print(member)
            embed = discord.Embed(title="Warn!", description="<@" + str(msg.author.id) +
                                  "> has warned <@" + str(member.id) + ">! \n\n**" + message + "**", color=0x00ff00)
            if member.id == msg.author.id:
                return
            embed.set_thumbnail(
                url="https://image.freepik.com/free-photo/judge-gavel-hammer-justice-law-concept_43403-625.jpg")
    ...
    if message.startswith(p + "mute"):
        ...
        for member in x.mentions:
            print(member)
            embed = discord.Embed(title="Muted!", description="<@" + str(msg.author.id) +
                                  "> has muted <@" + str(member.id) + ">! \n\n**" + message + "**", color=0x00ff00)
            if member.id == msg.author.id:
                return
