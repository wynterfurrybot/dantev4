...
async def msg(message, x, p, self):
    ...
    if message.startswith(p + "snowball"):
        for member in x.mentions:
            print(member)
            embed = discord.Embed(title="Snowball!", description="<@" + str(msg.author.id) +
                                  "> has thrown a snowball directly at <@" + str(member.id) + ">'s face!", color=0x00ff00)
            if member.id == msg.author.id:
                return
            embed.set_thumbnail(
                url="http://d.facdn.net/art/olypixandstuff/1265692624/1265692624.olypixandstuff_1263408058.sapphwolf_snowballfight2.png")
            await msg.channel.send(embed=embed)

    if message == p + "advent":
        items = ["Chocolate Reindeer", "Chocolate Snowflake",
                 "Chocolate Elf", "Chocolate Santa", "Chocolate Christmas Tree"]
