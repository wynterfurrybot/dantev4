...
async def msg(message, x, p, self):
    ...
    if message == p + "s":
        ...
        embed.add_field(name="Date of creation", value=msg.guild.created_at.__format__(
    ...
    if message.startswith(p + "u"):
        for user in x.mentions:
            embed = discord.Embed(description=str(
                user.name) + "#" + str(user.discriminator), color=0x00ff00)
            embed.set_thumbnail(url=user.avatar_url)
            embed.add_field(name="Name:", value=user.display_name, inline=True)
            embed.add_field(name='Account Created', value=user.created_at.__format__(
                '%A, %d. %B %Y @ %H:%M:%S'), inline=True)
            embed.add_field(name="Date Joined:", value=user.joined_at.__format__(
                '%A, %d. %B %Y @ %H:%M:%S'), inline=True)
            await msg.channel.send(embed=embed)
