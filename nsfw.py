...
async def msg(message, x, p, self):
    ...
    if message.startswith(p + "yiff"):
        ...
        for member in x.mentions:
            print(member)
            embed = discord.Embed(title = "H..harder daddy!", description = "<@" + str(msg.author.id) + "> has yiffed <@" + str(member.id) + "> hard!" , color=0x00ff00)
    ...
    if message.startswith(p + "suck"):
        ...
        for member in x.mentions:
            if member.id == msg.author.id:
                return
            print(member)
            embed = discord.Embed(title = "Spitters are quitters.", description = "<@" + str(msg.author.id) + "> has sucked off <@" + str(member.id) + ">!" , color=0x00ff00)
            embed.set_thumbnail(url = "https://w1680.luscious.net/laise/342630/frrbq_22_01DBK73R7KYGJDX9X3D0972X8A.1680x0.jpg")
            await msg.channel.send(embed = embed)
